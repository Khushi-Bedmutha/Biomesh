...
const Marketplace = () => {
  ...
  const toggleSelectDataset = (id: string) => {
    if (!selectedDatasets.includes(id)) {
      ...
      setSelectedDatasets(selectedDatasets.filter(itemId => itemId !== id));
    }
  };
          ...
          <div className="flex items-center space-x-2">
            ...
            {selectedDatasets.length > 0 && (
              <button className="flex items-center bg-indigo-600 text-white px-4 py-2 rounded-lg hover:bg-indigo-700">
                ...
                Request ({selectedDatasets.length})
              </button>
            )}
          </div>
        </div>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                        <button
                          ...
                          className={`px-4 py-2 rounded-lg text-xs font-medium ${
                            selectedDatasets.includes(item.id)
                              ? 'bg-indigo-100 text-indigo-800 border border-indigo-300'
                              : 'border border-indigo-600 text-indigo-600 hover:bg-indigo-50'
                          }`}
                        >
                          {selectedDatasets.includes(item.id) ? 'Selected' : 'Select'}
                        </button>
                      </td>
                    </tr>
                  );
